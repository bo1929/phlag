...
cdef class QQS:
    ...
    cdef double f(self, w, x, y, z, gt):
        ...
        cdef double r = 0.0, q
        cdef list s = []
        for u in gt.traverse_postorder():
            if u.is_leaf():
                lbl = u.get_label()
                iw = PyDict_Contains(w, lbl)
                ix = PyDict_Contains(x, lbl)
                iy = PyDict_Contains(y, lbl)
                iz = PyDict_Contains(z, lbl)
            else:
    ...
    def get_qp(self, lbl):
        nd = self.lbl_to_nd[lbl]
        assert not nd.is_root() and not nd.is_leaf()
        assert(len(list(nd.child_nodes())) == 2)
        cdef object x, y, w, z
        x, y = (self.st.extract_subtree(child).traverse_postorder(internal=False) for child in nd.child_nodes())
        w = self.st.extract_subtree(nd.parent.child_nodes()[1 if nd.parent.child_nodes()[0] == nd else 0]).traverse_postorder(internal=False)
        # if nd.parent.parent is not None:
        #     w = self.st.extract_subtree(nd.parent.child_nodes()[1 if nd.parent.child_nodes()[0] == nd else 0]).traverse_postorder(internal=False)
        # else:
        #     sister = nd.parent.child_nodes()[1 if nd.parent.child_nodes()[0] == nd else 0]
        #     w = self.st.extract_subtree(sister.child_nodes()[0]).traverse_postorder(internal=False)
        z = self.st.traverse_postorder(internal=False)
        get_lbls = lambda s: set(nd.get_label() for nd in s)
        x, y, w, z = get_lbls(x), get_lbls(y), get_lbls(w), get_lbls(z)
        z = z - x - y - w
        make_dict = lambda s: {lbl : True for lbl in s}
        # xstr = str(",".join(list(x)))
        # ystr = str(",".join(list(y)))
        # wstr = str(",".join(list(w)))
        # zstr = str(",".join(list(z)))
        # qstr = lbl + "\t(" + xstr + ")+(" + ystr + ") | (" + wstr + ")+(" + zstr + ")"
        # print(qstr)
    ...
    def freq(self, x, y, w, z, gt, normalize=True):
        f1, f2, f3 = (self.f(x, y, w, z, gt),
                      self.f(x, w, y, z, gt),
                      self.f(x, z, w, y, gt))
        if not normalize:
            return (f1, f2, f3)
...
cdef class MSC:
    ...
    cdef object qqs, st, gene_trees
    cdef int num_gene_trees

    def __init__(self, st, gene_trees):
        self.st = st
    ...
    def compute_qqs(self):
        cdef object lbl_to_qqs = defaultdict(partial(jnp.zeros, (self.num_gene_trees, 3)))
        cdef list internal_nd_l = [
            nd for nd in self.st.traverse_postorder(leaves=False)
            if not nd.is_root() and not nd.is_leaf()
        ]
        cdef object nd, gt, x, y, w, z
        cdef double f1, f2, f3
        cdef str lbl
        cdef int i

        for nd in internal_nd_l:
            lbl = nd.get_label()
            x, y, w, z = self.qqs.get_qp(nd.get_label())
            if min(len(x), len(y), len(w), len(z)) >= 1:
                for i in range(self.num_gene_trees):
                    f1, f2, f3 = self.qqs.freq(x, y, w, z, self.gene_trees[i])
                    lbl_to_qqs[lbl] = lbl_to_qqs[lbl].at[i, :].set((f1, f2, f3))
        return lbl_to_qqs

    @cython.boundscheck(False)
    @cython.wraparound(False)
    def simulate_qqs(self, clade_lbls, num_reps):
        cdef object tns = dendropy.TaxonNamespace()
        cdef object dendropy_tree = dendropy.Tree.get(
            data=self.st.newick(), schema="newick", taxon_namespace=tns
        )
        ...
        cdef object simulated_qqs = jnp.zeros((num_reps, len(clade_lbls), 3))
        ...
        for i in range(int(num_reps)):
            ...
            for nd in simulated_tree.traverse_leaves():
                parts = nd.get_label().split("_")
                nd.set_label("_".join(parts[:len(parts)-1]))
            j = 0
            for clade in clade_lbls:
                x, y, w, z = self.qqs.get_qp(clade)
                assert min(len(x), len(y), len(w), len(z)) >= 1
                f1, f2, f3 = self.qqs.freq(x, y, w, z, simulated_tree)
                simulated_qqs = simulated_qqs.at[i, j, :].set((f1, f2, f3))
                j += 1

        return simulated_qqs
